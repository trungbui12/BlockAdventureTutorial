...
public class Shape : MonoBehaviour, IPointerClickHandler, IPointerUpHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerDownHandler
{
    ...
    {
        if(IsOnStartPosition() == false && IsAnyOfShapeSquaresActive())
        {
             foreach (var square in _currentShape)
            {
    ...
    public void CreateShape(ShapeData shapeData)
    {
        ...
        foreach ( var square in _currentShape)
        {
            square.GetComponent<RectTransform>().localPosition = Vector3.zero;
            square.gameObject.SetActive(false);
        }
        ...
        for (var row = 0; row < shapeData.rows; row++)
        {
            for (var column = 0; column < shapeData.columns; column++)
            {
                if(shapeData.board[row].column[column])
                {
                    ...
                    float x = (column - (shapeData.columns - 1) / 2f) * moveDistance.x;
                    float y = ((shapeData.rows - 1) / 2f - row) * moveDistance.y;
     ...
     if (shapeData.columns > 1) //vertical position calculation
        {
            if (shapeData.columns % 2 != 0)
            {
                ...
                if (column < middleSquareIndex) // move it on negative
                {
                    shiftOnX = moveDistance.x * -1;
                    shiftOnX *= multiplier;
                }
                else if (column > middleSquareIndex) // move it on positive
                {
                    shiftOnX = moveDistance.x * 1;
                    shiftOnX *= multiplier;
                }
